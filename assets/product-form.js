...
class ProductFormComponent extends Component {
  ...
  const cartItemsComponent = document.querySelector(`cart-items-component[data-section-id="${id}"]`);
  if (cartItemsComponent) {
    // Parse the new HTML
    const tempDiv = document.createElement('div');
    tempDiv.innerHTML = html;

    const newCartItemsComponent = tempDiv.querySelector('cart-items-component');
    if (newCartItemsComponent) {
      // Replace old component with new one
      cartItemsComponent.replaceWith(newCartItemsComponent);

      // Re-initialize JS event listeners inside the new cart-items-component
      // Example: quantity buttons, remove buttons, etc.
      reinitializeCartItemsComponent(newCartItemsComponent);
    }
  }
});

// Example re-init function
function reinitializeCartItemsComponent(component) {
  // Quantity change buttons
  const qtyInputs = component.querySelectorAll('.cart-item__quantity input');
  qtyInputs.forEach((input) => {
    input.addEventListener('change', (e) => {
      console.log('Quantity changed', e.target.value);
      // Call your update cart logic here
    });
  });

  // Remove buttons
  const removeBtns = component.querySelectorAll('.cart-item__remove');
  removeBtns.forEach((btn) => {
    btn.addEventListener('click', (e) => {
      e.preventDefault();
      console.log('Item removed');
      // Call your remove item logic here
    });
  });

  // Add any other event listeners that your cart component needs
}
